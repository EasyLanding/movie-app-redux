...
const MovieContent = () => {
    ...
    let searchDataUp = useSelector((state: any) => state.repos.movieElement)
    ...
    let searchMovie = ((searchData: any) => {
        ...
        if (searchDataUp) {
            ...
            dispatch(setTotalPage((state: any) => state.repos.movieElement.length / pageSize))
    ...
    const changeRateMovie = (id: number) => {

        let idx = searchDataUp.findIndex((el: any) => el.id === id)
    ...
    const shortDescriptionMovie = (item: string) => {
        let maxLength = 28;
    ...
    const movieRaitingNumber = (item: number) => {
        let ratingColor
    ...
    let errorMessage = useSelector((state: any) => state.repos.error) ? < ErrorIndicator /> : null;
    let length = useSelector((state: any) => state.repos.movieElement.length)
    let tabRated = useSelector((state: any) => state.repos.tabRated)
    let loading = useSelector((state: any) => state.repos.loading)
        ...
        < div className='conteiner-1' >
            {
                tabRated ? <MovieContentCookie
                    ...
                    toggleTabRated={() => dispatch(setTabRated(true))}
                    idxCookie={idxCookie}
                                ...
                                <div className='movieCartConteiner'>
                                    {
                                        <div className="movieCart">
                                            {
                                                movieElement.map((itemTitle: any, index: number) => {
